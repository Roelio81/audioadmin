...
, m_currentTab(0)
{
    ...
    connect(m_ui.m_klantMutualiteit, SIGNAL(currentIndexChanged(int)), &m_file, SLOT(showInsuranceCompany(int)));
    connect(m_ui.b_klantBrief, SIGNAL(clicked()), &m_file, SIGNAL(showLetterCustomer()));
    connect(m_ui.b_artsBrief, SIGNAL(clicked()), &m_file, SIGNAL(showLetterPhysician()));
    connect(m_ui.b_mutualiteitBrief, SIGNAL(clicked()), &m_file, SIGNAL(showLetterInsuranceCompany()));
    connect(m_ui.b_factuur, SIGNAL(clicked()), &m_file, SIGNAL(showInvoice()));
    connect(m_ui.b_meetgegevens, SIGNAL(clicked()), &m_file, SIGNAL(showMeasurements()));
...
void Universe::tabChanged(int nieuweTab)
{
    switch (m_currentTab)
    {
...
{
    PriceDatePair info(prijs, datumPrijs);
...
void Universe::addInsuranceCompany(int id, const QString &naam, const QString &straat, int postcode, const QString &gemeente)
{
    ...
    changeInsuranceCompany(id, naam, straat, postcode, gemeente);
}

void Universe::changeInsuranceCompany(int id, const QString &naam, const QString &straat, int postcode, const QString &gemeente)
{
